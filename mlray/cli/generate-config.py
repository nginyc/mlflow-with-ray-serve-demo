...
from mlray.config import read_config
...
def main(
    ...
    ray_serve_config = build_ray_serve_config(mlflow_tracking_uri, deployable_models)
...
def update_kuberay_config(
    ...
    serve_config_yaml = yaml.safe_dump(serve_config, default_flow_style=False)
    
    # Update the spec.serveConfigV2 field
    if 'spec' not in kuberay_config:
        kuberay_config['spec'] = {}
    kuberay_config['spec']['serveConfigV2'] = serve_config_yaml
    
    # Write the updated config back to the file
    with open(kuberay_config_path, 'w') as f:
        yaml.safe_dump(kuberay_config, f, default_flow_style=False)
...
def build_ray_serve_config(
    ...
    for model in deployable_models:
        app = build_ray_serve_config_application(mlflow_tracking_uri, model)
